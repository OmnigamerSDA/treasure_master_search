...
void RNG::generate_alg0_bits_8()
{
	if (alg0_bits_8 != nullptr) return;
	// alg0: insert bit = (run_rng() >> 7) & 1 at state byte position (127-j)
	// for j=0..127.  Store as a 128-bit (16-byte) bitfield per seed.
	alg0_bits_8 = (uint8*)aligned_malloc(0x10000 * 16, 64);
	uint16 rng_seed;
	for (int i = 0; i < 0x10000; i++)
	{
		rng_seed = i;
		uint8* row = alg0_bits_8 + i * 16;
		for (int b = 0; b < 16; b++) row[b] = 0;
		for (int j = 0; j < 128; j++)
		{
			int offset = 127 - j;  // matches _generate_alg0_values store order
			uint8 bit = (run_rng(&rng_seed) >> 7) & 0x01;
			row[offset >> 3] |= bit << (offset & 7);
		}
	}
}
...
void RNG::generate_alg6_bits_8()
{
	if (alg6_bits_8 != nullptr) return;
	// alg6: insert bit = (run_rng() >> 7) & 1 at state byte position j
	// for j=0..127.  Store as a 128-bit (16-byte) bitfield per seed.
	alg6_bits_8 = (uint8*)aligned_malloc(0x10000 * 16, 64);
	uint16 rng_seed;
	for (int i = 0; i < 0x10000; i++)
	{
		rng_seed = i;
		uint8* row = alg6_bits_8 + i * 16;
		for (int b = 0; b < 16; b++) row[b] = 0;
		for (int j = 0; j < 128; j++)
		{
			int offset = j;  // matches _generate_alg6_values store order (forward)
			uint8 bit = (run_rng(&rng_seed) >> 7) & 0x01;
			row[offset >> 3] |= bit << (offset & 7);
		}
	}
...
void RNG::cleanup()
{
	...
	aligned_free(alg6_bits_8);                  alg6_bits_8 = nullptr;
